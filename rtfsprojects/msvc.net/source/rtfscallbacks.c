...
/* Need to make this a registered callback driver so we don't need conditional compilation 
	This is replaced by device driver self registrion via pc_rtfs_register_poll_devices_ready_handler 
			PollDeviceReady();
*/
#endif
#endif
...
long line_number = 0;
	...
	switch (cb_code)
	{
	    ...
	    case RTFS_CBD_IOERROR:   /* iarg0 contains the drive number */
			break;
//	    case RTFS_CBD_SETERRNOVERBOSE:  /* iarg0 contains the error value */
	    case RTFS_CBD_SETERRNO:  /* iarg0 contains the error value */
	    	//if (cb_code == RTFS_CBD_SETERRNO)
				 error_number = iarg0;
...
//int fs_flush_behavior = FS_CB_FLUSH;		/* Tell Failsafe to flush the journal file but not synchronize the FAT volume*/
// int fs_flush_behavior = FS_CB_CONTIINUE;	/* Tell Failsafe to not flush the journal file and not syncronize */
