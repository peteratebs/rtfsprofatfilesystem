...
DRIVE_INFO drive_info_stats;
	...
	if (drive_info_stats.fat_entry_size == 12 || drive_info_stats.is_exfat)
        return(TRUE);
    return(FALSE);
}
...
BOOLEAN pro_test_compare_checkpoints(PRO_TEST_CHECKPOINT *pcheck1,PRO_TEST_CHECKPOINT *pcheck2)
{
    if
    (
	pcheck1->bam_cksum !=  pcheck2->bam_cksum ||
...
DDRIVE *pdr;
    ...
    dword i, n_blocks_left, block_no, n_blocks;
		n_blocks = pdr->exfatextension.BitMapBufferSizeSectors;
		block_no =  pc_cl2sector(pdr,pdr->exfatextension.FirstClusterOfBitMap[0]);
        ...
        byte *puser_buff;
            ...
            pcheck->bam_cksum += pro_test_checksum(puser_buff+8, test_drive_structure()->drive_info.bytespsector-8);
            ...
            {
                 pcheck->bam_cksum += pro_test_checksum(puser_buff, test_drive_structure()->drive_info.bytespsector);
            }
            n_blocks_left -=  blocks_to_read;
            block_no += blocks_to_read;
        }
		/* In exFAT the checksum of the FAT is not a reliable indicator because a cluster's
		   value in the BAM superceeds the value in the FAT */
		goto exit_here;
	}
	else
#endif
    ...
    dword i, n_blocks_left, block_no, n_blocks;
        ...
        byte *puser_buff;
            ...
            puser_buff += test_drive_structure()->drive_info.bytespsector;
#if (INCLUDE_EXFAT)
         /* For exfat checksum the last sector of the FAT.. we will write the journal info here */
			if (ISEXFAT(pdr))
			{
				if (blocks_to_read >= n_blocks_left)
				{
					if (blocks_to_read)
						blocks_to_read-=1;
					if (blocks_to_read==0)
...
void pro_test_release_mount_parameters(DDRIVE *ptest_drive)
{
	if (test_dsk_config_active)
	{
		...
		pc_dskfree(ptest_drive->driveno);
		pc_rtfs_free(ptest_drive->pmedia_info->device_sector_buffer_base);
		ptest_drive->pmedia_info->device_sector_buffer_base=0;
