...
static byte ide_pcmcia_cfg_opt_value=0;
static dword ide_register_file_address;
static dword ide_interrupt_number;
//pdr->controller_number))
//pdr->drive_flags & DRIVE_FLAGS_CDFS)
//pdr->driveno, DEVCTL_REPORT_REMOVE, (void *) 0);
//pdr->interrupt_number,
//pdr->logical_unit_number)                             /* Bit four is drive (1 or 0) */
//pdr->pcmcia_cfg_opt_value  == 0x41 || pdr->pcmcia_cfg_opt_value  == 0x40)
...
BOOLEAN _ide_io(DDRIVE *pdr, dword sector, void *buffer, word count, BOOLEAN reading)    /*__fn__*/
{
BOOLEAN ret_val;
PIDE_CONTROLLER pc;
...
BOOLEAN ide_io(DDRIVE *pdr, dword sector, void *buffer, word count, BOOLEAN reading)
{
    byte *b;
    b = (byte *) buffer;
    ...
    {
        if (! _ide_io(pdr, sector, b, count, reading))
            return(FALSE);
        return(TRUE);
    }
    ...
    {
           if (! _ide_io(pdr, sector, b, MAX_BLOCKS, reading))
                return(FALSE);
            sector += MAX_BLOCKS;
            b += (MAX_BLOCKS*512);
            count -= MAX_BLOCKS;
...
static int ide_perform_device_ioctl(DDRIVE *pdr, int opcode, void * pargs)
{
   ...
   a card removal interrupt is not available */
            ...
            else if (pdr->drive_flags & DRIVE_FLAGS_PCMCIA)
            {
                ...
                if (pcmcia_card_is_ata(
                    ...
                    ide_register_file_address,
                    ide_interrupt_number,
                    ide_pcmcia_cfg_opt_value) )
...
/*            break; */
        case DEVCTL_WARMSTART:
            if (!prtfs_cfg->ide_semaphore)
                prtfs_cfg->ide_semaphore = pc_rtfs_alloc_mutex("IdeDrive");
            if (!prtfs_cfg->ide_semaphore)
                return(-1);
            if (pc->init_signature != CONTROLLER_INITIALIZED)    /* Only initialize the controller once */
            {
                ...
                pc->register_file_address = ide_register_file_address;

                pc->interrupt_number = ide_interrupt_number;
            ...
            if (pdr->drive_flags & DRIVE_FLAGS_PCMCIA)
            {
                ...
                if (ide_pcmcia_cfg_opt_value  == 0x41 || ide_pcmcia_cfg_opt_value  == 0x40)
...
DDRIVE *pdr;
    ...
    for (j = 0; j < prtfs_cfg->cfg_NDRIVES; j++)
    {
        ...
        {
            ide_perform_device_ioctl(pdr, DEVCTL_REPORT_REMOVE, (void *) 0);
            break;
        }
    }
}
...
static int BLK_DEV_IDE_blkmedia_io(void  *devhandle, void *pdrive, dword sector, void  *buffer, dword count, BOOLEAN reading);
static int  BLK_DEV_IDE_blkmedia_ioctl(void  *handle_or_drive, void *pdrive, int opcode, int iArgs, void *vargs);

/* Sector buffer provided by BLK_DEV_IDE_Ramdisk_Mount */
static byte idedisk_sectorbuffer[512];

static DDRIVE dummy_ide_drivestructure;



#define DEFAULT_NUM_SECTOR_BUFFERS  32
#define DEFAULT_NUM_DIR_BUFFERS     32
#define DEFAULT_NUM_FAT_BUFFERS     4
#define DEFAULT_FATBUFFER_PAGE      8
...
static int BLK_DEV_IDE_configure_device(struct rtfs_media_insert_args *pmedia_parms, struct rtfs_media_resource_reply *pmedia_config_block, int sector_buffer_required)
{
    RTFS_ARGSUSED_PVOID((void *)pmedia_parms);

    rtfs_print_one_string((byte *)"Attaching a Ide Disk driver at drive D:", PRFLG_NL);

	pmedia_config_block->requested_driveid = (int) ('D'-'A');
	...
	{
		pmedia_config_block->device_sector_buffer_base = (byte *) &idedisk_sectorbuffer[0];
		pmedia_config_block->device_sector_buffer_data  = (void *) &idedisk_sectorbuffer[0];
	}
	/*	0  if successful
...
static int BLK_DEV_IDE_configure_volume(struct rtfs_volume_resource_request *prequest_block, struct rtfs_volume_resource_reply *pvolume_config_block)
{
...
/* Call after Rtfs is intialized to start a ide disk driver on A: */
BOOLEAN BLK_DEV_IDE_Insert(void)
{
    /* Set up mount parameters and call Rtfs to mount the device    */
struct rtfs_media_insert_args rtfs_insert_parms;
DEV_GEOMETRY gc;

    if (ide_perform_device_ioctl(&dummy_ide_drivestructure, DEVCTL_WARMSTART, (void *) 0)<0)
		return FALSE;

    /* register with Rtfs File System */
    rtfs_insert_parms.devhandle = (void *) &dummy_ide_drivestructure; /* Not used just a handle */
    ...
    ide_perform_device_ioctl(&dummy_ide_drivestructure, DEVCTL_GET_GEOMETRY, (void *) &gc);
    ...
    rtfs_insert_parms.device_io                = BLK_DEV_IDE_blkmedia_io;
    rtfs_insert_parms.device_ioctl             = BLK_DEV_IDE_blkmedia_ioctl;
    rtfs_insert_parms.device_erase             = 0;
    rtfs_insert_parms.device_configure_media    = BLK_DEV_IDE_configure_device;
    rtfs_insert_parms.device_configure_volume   = BLK_DEV_IDE_configure_volume;
...
static RTFS_DEVI_POLL_REQUEST_VECTOR poll_device_ide_storage;
static void PollIdeDeviceReady(void);

BOOLEAN BLK_DEV_IDE_Idedisk_Mount(void)
{

	dummy_ide_drivestructure.logical_unit_number=0;
	dummy_ide_drivestructure.drive_flags = 0;
	dummy_ide_drivestructure.controller_number = 0;
    /* To retrieve configuration values from old implentation see: apiinit.c(attach_one_ide_device()) */
 	ide_interrupt_number = 14;          /* Standard interrupt number (-1 for polled) */
	ide_register_file_address=0x1f0;    /* Standard IO address for primary controller */
	ide_pcmcia_cfg_opt_value  = 0x0; // ??


#if (SUPPORT_HOTSWAP)
	pc_rtfs_register_poll_devices_ready_handler(&poll_device_ide_storage, PollIdeDeviceReady);
	if (ide_perform_device_ioctl(&dummy_ide_drivestructure,  DEVCTL_CHECKSTATUS, (void *) &dummy_ide_drivestructure)==DEVTEST_NOMEDIA)
		return TRUE;
#endif
	return BLK_DEV_IDE_Insert();
}
static int BLK_DEV_IDE_blkmedia_io(void  *devhandle, void *pdrive, dword sector, void  *buffer, dword count, BOOLEAN reading)
{
    ...
    return (int) ide_io(&dummy_ide_drivestructure, sector, buffer, (word)count, reading);
}


static int  BLK_DEV_IDE_blkmedia_ioctl(void  *handle, void *pdrive, int opcode, int iArgs, void *vargs)
{
...
#if (SUPPORT_HOTSWAP)
static void PollIdeDeviceReady()
{
	if (ide_perform_device_ioctl(&dummy_ide_drivestructure, DEVCTL_CHECKSTATUS, (void *) &dummy_ide_drivestructure)==DEVTEST_CHANGED)
		BLK_DEV_IDE_Insert();
}
#endif
