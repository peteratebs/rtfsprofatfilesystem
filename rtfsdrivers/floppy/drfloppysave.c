...
BOOLEAN floppy_io(int driveno, dword l_sector, void  *in_buffer, word count, BOOLEAN reading)   /*__fn__*/
{
int i;
DDRIVE *pdr;
...
int floppy_perform_device_ioctl(int driveno, int opcode, void * pargs)
{
DDRIVE *pdr;
...
byte fltype;
    ...
    switch (opcode)
    {
        ...
        rtfs_cs_strcpy(&gc.fmt.oemname[0], pustring_sys_oemname, CS_CHARSET_NOT_UNICODE);
        gc.fmt.physical_drive_no =   0;
        gc.fmt.binary_volume_label = BIN_VOL_LABEL;
        rtfs_cs_strcpy(gc.fmt.text_volume_label,pustring_sys_volume_label, CS_CHARSET_NOT_UNICODE);
        ...
        case DEVCTL_WARMSTART:
            if (!prtfs_cfg->floppy_semaphore)
                prtfs_cfg->floppy_semaphore = pc_rtfs_alloc_mutex();
            if (!prtfs_cfg->floppy_semaphore)
                return(-1);
            shadow_dig_out = 0;
            shut_mo_off_in = 0;
