...
static void fs_consume_frames_ex(byte * fname, byte * fname2, dword nframes);
...
FAILSAFECONTEXT *pfstest_failsafe_context;
    ...
    test_size_clusters = minimum_journal_size * 8;
    {
        ...
        /* Now consume blocks until the file is reduce to minimum_journal_size blocks. The journal needs at least 3 sectors */
		...
		{
		dword n_toburn,nresidual_to_expext;
			if (sec_p_cluster == 1) /* Will be 2, 4, 8 */
			{
				n_toburn = test_size_clusters-5;
				nresidual_to_expext = 3;
			}
			else if (sec_p_cluster == 2)
			{
				n_toburn = test_size_clusters-4;
				nresidual_to_expext = 2;
			}
			else
			{
				n_toburn = test_size_clusters-3;
				nresidual_to_expext = 1;
			}


			if (_fill_using_file((byte *)"DISK_FULL_TEST_TWO_FILE_NAME", bytes_per_cluster, n_toburn,0 ) != n_toburn)
			{ ERTFS_ASSERT_TEST(rtfs_debug_zero()) }

			/* The journal should be 1 cluster now */
			if ((pfstest_failsafe_context->nv_reserved_fragment.end_location - pfstest_failsafe_context->nv_reserved_fragment.start_location+1) != nresidual_to_expext)
			{ ERTFS_ASSERT_TEST(rtfs_debug_zero()) }
		}
...
static void fs_consume_frames(byte * fname, dword nframes)
{
	fs_consume_frames_ex(fname, 0, nframes);
}
...
byte attr,attr2;
	...
	if (fname2)
	{
		if (!pc_get_attributes(fname2, &attr2))
			{ ERTFS_ASSERT_TEST(rtfs_debug_zero()) }
	}
    while (frames_left >= 2)
    {
        ...
        { ERTFS_ASSERT_TEST(rtfs_debug_zero()) }
		/* Obscure - if an odd number consume 3 blocks instead of 2 by updating a dirent known to be
		   in a different block. We know this because fname2 is the directory containing
		   fname 1 and can not be in the same block */
		if (fname2 && frames_left & 1)
		{
			if (!pc_set_attributes(fname2, attr2))
				{ ERTFS_ASSERT_TEST(rtfs_debug_zero()) }
			frames_left -= 1;
		}
...
#define FSTESTDIR "FSTEST"
#define FSTESTF1  "FSTEST\\FSTEST_1"
#define FSTESTF2  "FSTEST\\FSTEST_2"

#define WRAPTESTFILESIZE 128
...
FAILSAFECONTEXT *pfstest_failsafe_context;
    ...
    fstest_simple_drive_config(DRVPOL_DISABLE_AUTOFAILSAFE, DEFAULT_BLOCKMAPSIZE,  DEFAULT_INDEX_BUFFER_SIZE ,DEFAULT_RESTORE_BUFFER_SIZE, WRAPTESTFILESIZE);


    /* don't use < 2 in this loop because fs_consume_frames() requires >= 2
       don't go beyond WRAPTESTFILESIZE-3 because we use leading_replacement_records + 2 records and on a
       wrap we can lose a record */
    for (leading_replacement_records = 2; leading_replacement_records < WRAPTESTFILESIZE-3; leading_replacement_records += 1)
    {
        ...
        fs_api_disable(test_driveid, TRUE);
		/* Create or recreate a directory to run the test in. */
		pc_mkdir((byte *)FSTESTDIR);
        pc_unlink((byte *)FSTESTF1);
        pc_unlink((byte *)FSTESTF2);

        /* create an empty 32 bit file */
        fd = create_file_for_fstest((byte *)FSTESTF1);
        ERTFS_ASSERT_TEST(fd >= 0)
        if (!close_file_for_fstest(fd))
        { ERTFS_ASSERT_TEST(rtfs_debug_zero()) }
        ...
        fs_consume_frames_ex((byte *)FSTESTF1, (byte *)FSTESTDIR ,leading_replacement_records);
        ...
        fs_consume_frames_ex((byte *)FSTESTF1, (byte *)FSTESTDIR, 2);
        ...
        { ERTFS_ASSERT_TEST(rtfs_debug_zero()) }
#endif
        /* Now consume all but 4 frames to forces a wrap. */
		pfstest_failsafe_context = fstest_failsafe_context();

        fs_consume_frames_ex((byte *)FSTESTF1, (byte *)FSTESTDIR , pfstest_failsafe_context->fs_journal.frames_free - 4);

        /* No create another empty 32 bit file */
        fd = create_file_for_fstest((byte *)FSTESTF2);
        ERTFS_ASSERT_TEST(fd >= 0)
        if (!close_file_for_fstest(fd))
        { ERTFS_ASSERT_TEST(rtfs_debug_zero()) }
		...
		{
        	RTFS_PRINT_STRING_1((byte *)"Informational: Session didn't wrap at this offset : ", PRFLG_NL);
		}

        /* the new file should not be there */
        if (pc_stat((byte *)FSTESTF2, &st) == 0)
        { ERTFS_ASSERT_TEST(rtfs_debug_zero()) }
