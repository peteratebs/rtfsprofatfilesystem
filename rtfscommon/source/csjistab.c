...
*
* Copyright EBS Inc. 2002,2004
* All rights reserved.
* This code may not be redistributed in source or linkable object form
* without the consent of its author.
*/
/* JISTAB.C - Contains tables to convert JIS to unicode and reverse.
              Note that these tables are actually Microsoft's CP932
              (also known as Windows-31J).  They contain
              JIS X 0201-1997 single-byte characters (159 characters),
              JIS X 0211-1994 single-byte characters (32 characters),
              JIS X 0211-1997 double-byte characters (6879 characters),
              NEC special characters (83 characters),
              NEC-selected IBM extended characters (374 characters),
              and IBM extended characters (388 characters).
              Microsoft's recommended round-trip mapping found in
              Q170559 is used.
...
/* There are 398 non-round-trip mappings.  See Microsoft's Q170559. */
#define NUMJISCHARS 7724
#define NUMUNICHARS 7326


/* UCODE, JIS */
KS_CONSTANT word ucode2jis[NUMUNICHARS][2] = {
...
/* JIS UCODE */
KS_CONSTANT word jis2ucode[NUMJISCHARS][2] = {
...
/* Convert a unicode string to a JIS string */


/* Character to use is JIS to Unicode lookup fails */
#define UNICODEDEFAULTCHAR (word) 0x005f /* '_' UNICODE_UNDERBAR */
...
byte c;
word w, w1;
word j; /* JIS character as a word */

    /* Convert JIS to word. Use one or two bytes of JIS */
    if (*p >= 0xa1 && *p <= 0xdf) /* Katakana */
    {
        c =  *p;
        w = (word) c;
        w += 0xfec0;
    ...
    else if (*p < 0x80) /* Ascii - not sure check if upper or lower */
    {
        w = (word) *p;
    ...
    /* The rest are two byte codes */
    w = (word) *p++;
    w1 = (word) *p;

    j = (word)((w << 8) | (w1));

    /* Now binary search the jis2ucode look up table */
    low=0;
    high=NUMJISCHARS-1;
    ...
    while (low < high)
    {
        ...
        if (j == jis2ucode[midpoint][0])
        {
            w = jis2ucode[midpoint][1];
            goto return_val;
        }
    ...
    if (j == jis2ucode[low][0])
    {
        w = jis2ucode[low][1];
        goto return_val;
    }

    /* No match, return a default character */
    w = UNICODEDEFAULTCHAR;

return_val:
    /* Make sure in intel order */
    fr_WORD (to,  w);
}

/* Character to use is Unicode to JIS fails */
#define JISDEFAULTCHAR (word) '_'
...
int low, high, midpoint;
word w,w1;
word u; /* Unicode character we want to convert to JIS */

    /* little endian unicode - lowbye|hibyte */
    w = (word) *punicode++;
    w1  = (word) *punicode;

    u = (word)((w1 << 8) | (w));
    /* Check for 1 byte jis */
    ...
    if (u == ucode2jis[low][0])
    {
        w = ucode2jis[low][1];
        goto return_val;
    }

    *pjis = JISDEFAULTCHAR; /* No match */
    *is_default = TRUE;
    return(1);
return_val:
