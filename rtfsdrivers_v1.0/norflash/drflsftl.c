...
#define STATIC_SECTOR_MAP_SIZE 128
...
int flashdisk_perform_device_ioctl(int driveno, int opcode, void * pargs)
{
DDRIVE *pdr;
DEV_GEOMETRY gc;        /* used by DEVCTL_GET_GEOMETRY */
    ...
    case DEVCTL_GET_GEOMETRY:
        ...
        if (FlashData.TotalSectors && FlashData.SectorsPerBlock)
        {
          gc.dev_geometry_heads       = 1;
          gc.dev_geometry_cylinders   = (dword) FlashData.TotalSectors/FlashData.SectorsPerBlock;
          gc.dev_geometry_secptrack   = (int) FlashData.SectorsPerBlock;
        }
        else
            return (-1);
...
#endif
        MapSectors((void *) &FlashData);
        if (FlashData.TotalSectors)
        {
           dword PhysicalSector;
           pdr->drive_flags |= DRIVE_FLAGS_VALID;
           /* If you can map block zero it is probably formatted */
           PhysicalSector = LocateSector(&FlashData, 0);
           if (PhysicalSector > FlashData.TotalSectors) /* bad equals: 0xffffff */
           {
               /* Force format block 0 not mapped  */
...
static int  MapSectors(void * DriveData)
{
   ...
   if(!D->SectorsAreMapped)
    {
       Result = mtd_MountDevice(D, &D->FlashInfo);
